...
/**
 * What we store in the Cache as a File
 *
 * @typedef {Object} CachedFile
 *
 * @prop {number} last_used The Epoch time it was last read from
 * @prop {number} mtimeMs The Epoch time the file was editied
 * @prop {string} file The actual file contents
 */
 ...
 * @prop {function(string, CachedFile): Promise<void>} set
...
module.exports.create = ({ max_length = 100, log }) => {
  ...
  */
   ...
   * 
   * @param {string} path 
   * @param {CachedFile} info
   */
  const set = async (path, info) => {
    cache.set(path, info)
