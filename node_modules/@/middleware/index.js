 ...
 */
module.exports.log_request = (log_level = 'trace') => async (ctx, next) => {
  utils.invariant(
    log_level in ctx.log,
    `Must give log_request a log_level that is a known log level, given ${log_level} instead`
  )
  // Let the downstream handlers figure out if
  // they can handle it or not
  await next()

  ctx.log[log_level]({ req: ctx.req, res: ctx.res, status: ctx.status })
}
...
module.exports.static_files = ({ public, index = true }) => async (ctx, next) => {
  ...
  try {
    ...
    if (e.message.indexOf("Cannot Read File That Doesn't Exist")) {
      // if we want this middleware to just throw,
      // we can uncomment this
      // throw new ctx.errors.NotFound()
      // however, I think it would be best to be able to say
      // that this just skips to the next one now that we know
      // we can't find it
      return next()
    } else {
      throw e
