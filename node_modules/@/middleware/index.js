...
const get_path = (possibleFilePath, index) => {
  ...
  if (index) {
    return get_path(possibleFilePath + 'index.html', index)
  }
}
...
module.exports.static_files = ({ public, index = true }) => async (ctx, next) => {
  // give anyone else a chance to respond
  await next()
  ...
  // If they have set the body/status already
...
module.exports.security_headers = () => (ctx, next) => {
  ...
  ctx.res.setHeader('X-OVERLORD', 'Tim Roberts <timroberts@fastmail.org>')
